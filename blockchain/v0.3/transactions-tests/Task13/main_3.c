/* Description: Mine a Block containing a coinbase transaction and a mock transaction and verify it, but the coinbase transaction is not the first in the list */
	...
	char const *data, EC_KEY *miner)
{
	...
	llist_add_node(block->transactions, transaction, ADD_NODE_FRONT);
