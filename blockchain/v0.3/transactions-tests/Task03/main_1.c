/* Description: Creates a transaction containing one input and two outputs, and compute its ID */
...
int main(void)
{
	 ...
	 * our `owner` will receive the coins froms
	 */
	...
	out2 = tx_out_create(900, pub);
	llist_add_node(transaction.outputs, out2, ADD_NODE_REAR);
	memset(pub, 57, EC_PUB_LEN); /* Pretend it's a different address :) */
	out2 = tx_out_create(72, pub);
	llist_add_node(transaction.outputs, out2, ADD_NODE_REAR);
