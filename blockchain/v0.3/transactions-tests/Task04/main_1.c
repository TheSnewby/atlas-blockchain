/* Description: Create a transaction with one input. Modify unspent output referenced by created input so owner's public key doesn't match. Try to sign the input */
...
int main(void)
{
	...
	unspent->out.pub[EC_PUB_LEN - 1] += 1;
